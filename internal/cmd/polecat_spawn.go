	...
	if findErr == nil && idlePolecat != nil {
		...
		if _, err := polecatMgr.ReuseIdlePolecat(polecatName, addOpts); err != nil {
			if errors.Is(err, polecat.ErrPolecatNotIdle) {
				// Concurrent sling already reused this polecat (hq-xqfo).
				// Skip repair and fall through to AllocateAndAdd.
				fmt.Printf("  Idle polecat %s was claimed by concurrent sling, allocating new...\n", polecatName)
			} else {
